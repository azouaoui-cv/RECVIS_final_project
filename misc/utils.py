...
def language_eval(dataset, preds, model_id, split, opt):
    ...
    if dataset == 'coco':
        ...
        annFile = "tools/coco-caption/annotations/caption_unrel.json"
...
# UnRel special: no GT bboxes!
class RandomCrop(object):
    """Crop the given PIL Image at a random location (NO GT BBOXES).
    Args:
        size (sequence or int): Desired output size of the crop. If size is an
            int instead of sequence like (h, w), a square crop (size, size) is
            made.
    ...
    def __call__(self, img, proposals):
        """
        Args:
            img (PIL Image): Image to be cropped.
            proposals: proposals to be cropped.
        Returns:
            PIL Image: Cropped image.
        """
        if self.padding > 0:
