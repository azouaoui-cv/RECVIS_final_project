...
class DataLoader(data.Dataset):
    def __init__(self, opt, split='train', seq_per_img=5):
        ...
        self.RandomCropWithBbox = utils.RandomCropWithBbox(opt.image_crop_size) # We do not need this one
        self.RandomCrop = utils.RandomCrop(opt.image_crop_size) # We use this one
        self.ToTensor = transforms.ToTensor()
        self.res_Normalize = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # We use this one
        ...
        # load the json file which contains additional information about the dataset   # THIS SHOULD WORK
        ...
        # TODO: Rewrite above section
        print("DataLoader loading proposal file: ", opt.proposal_h5) 
        with h5py.File(self.opt.proposal_h5, "r", driver="core") as h5_file:
            self.num_proposals = h5_file["num_proposals"][:]
            self.proposals = h5_file["proposals"][:]
    ...
    def __getitem__(self, index):
        ...
        # TODO: refactor above
        # load the proposal 
        #num_proposals = int(self.num_proposals[ix])
        num_proposals = int(self.num_proposals[image_id])
        #proposals = self.proposals[ix]
        ...
        img, proposals = self.RandomCrop(img, proposals)
        ...
        # TODO: Rewrite somehow the section above
        cap_seq = np.zeros((ncap, self.seq_length, 5))
        for i, caption in enumerate(captions):
            j = 0
            while j < len(caption) and j < self.seq_length:
                cap_seq[i, j, 0] = self.wtoi.get(caption[str(j)], self.wtoi["UNK"])
                cap_seq[i, j, 4] = self.wtoi.get(caption[str(j)], self.wtoi["UNK"])
                j += 1

        # TODO: handle missing word to UNK
        ...
        #input_seq[:,1:] = seq_batch # What is the input sequence used for?
        ...
        #pad_proposals = np.zeros((self.max_proposal, 6)) # 6 OR 4 here?
        ...
        if self.opt.det_oracle == False: # THIS IS OUR CASE HERE
        ...
        num = torch.FloatTensor([ncap, num_pps, 1.0]) # REMOVE num_box
